...
class Game {
    ...
    async initialize() {
        try {
            ...
            // تهيئة نظام الرسوميات
            this.graphics = new AdvancedGraphics(canvas);
    ...
    addTestCube() {
        try {
            ...
            // إضافة المكعب للمشهد
            this.graphics.add(this.cube);
        } catch (error) {
            console.error('Error adding test cube:', error);
            this.handleError(error);
        }
    }
    ...
    updateUI() {
        try {
            ...
            if (woodElement) woodElement.textContent = '0';
            if (metalElement) metalElement.textContent = '0';
            if (foodElement) foodElement.textContent = '0';
        } catch (error) {
